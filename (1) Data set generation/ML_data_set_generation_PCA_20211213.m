...
number_of_components_to_use = 30; % !!!!!!!!!! subject to change
dataSetNum = 2; %%%!!!!! intact
...
outputFolder = 'CS545_data_set_PCA_30_20211213_intact';
mkdir(outputFolder)
...
load('Spectrums - original and PCA/PCA_coeffs_on_spectrogram_whole.mat')
...
for number_measureNum = 1 : length_measureNumList
    ...
    spectrogram_r_i = [spectrogram_real; spectrogram_imag];
    spectrogram_r_i_T = spectrogram_r_i'; % transpose for PCA
    
    % zero-mean normalization
    mean_spectrogram_r_i_T = mean(spectrogram_r_i_T, 1);
    spectrogram_r_i_T_centered = spectrogram_r_i_T - mean_spectrogram_r_i_T;
    
    % calculate the lower-dimensional representation of each sample
    spectrogram_PCA = spectrogram_r_i_T_centered * PCA_coeff(:, 1:number_of_components_to_use);
    % transpose back to the form we are familiar in the course
    % the number of rows equals to the number of PCA components used
    spectrogram_PCA = spectrogram_PCA';
...
%         end
%         
%         spectrogram = spectrogram.T; % transpose for the PCA
%         % calculate the lower-dimensional representation of each sample
%         spectrogram_PCA = spectrogram * PCA_coeff(:, 1:number_of_components_to_use);
%         
%         % transpose back to the form we are familiar in the course
%         % the number of rows equals to the number of PCA components used
%         spectrogram_PCA = spectrogram_PCA';
