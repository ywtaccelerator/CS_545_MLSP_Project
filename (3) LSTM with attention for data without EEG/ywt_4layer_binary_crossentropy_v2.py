...
# #run with 8000 samples from y0 and y1
# training_set_y0 = training_set_y0[:, 0:4000] 
# training_set_y1 = training_set_y1[:, 0:4000]
...
for y0_feature in y0_feature_list:
    x_input.append(np.transpose(y0_feature))
    y_input.append(0)
del y0_feature_list
gc.collect()

for y1_feature in y1_feature_list:
    x_input.append(np.transpose(y1_feature))
    y_input.append(1)
del y1_feature_list
gc.collect()
...
x_input_tensor = np.zeros((len(x_input), time_steps, num_features))
for i in len(x_input):
    x_input_tensor[i,:,:] = x_input[i]

y_input_tensor = np.array(y_input)
...
for x_feature in x_feature_list:
    test_input.append(np.transpose(x_feature))
...
x_test_tensor = np.zeros((len(test_validation_x), time_steps, num_features))
for i in len(test_validation_x):
    x_test_tensor[i,:,:] = test_validation_x[i]
...
y_test_tensor = np.array(test_validation_y)
...
history = model.fit(x_input_tensor, y_input_tensor, validation_data=(x_test_tensor, y_test_tensor), epochs=my_num_epochs, batch_size=my_batch_size, verbose=2)
model.summary()
model.save("trained_LSTM_model_with_multiplayer.h5")

# Prediction
print("\n\nFitting finished!\n\n")
print("model.evaluate(x_test_tensor, y_test_tensor):\n")
print(model.evaluate(x_test_tensor, y_test_tensor))
print('\n\n')

test_validation_x = x_test_tensor
test_validation_y = y_test_tensor
